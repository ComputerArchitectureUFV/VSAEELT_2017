...
int recorde1 = 0, oldrecorde, recordeMAX;
// ---------------
// draw pixel
// ---------------
...
void game_loop() {
  ...
  while (1) {
    ...
    // if bird has passed the pipe increase score
      ...
      if (score > recorde1) {
        recorde1 = score;
        if (EEPROM.read(0)==0 && recorde1 > EEPROM.read(0)){
            EEPROM.write(0, recorde1);
        }
        
        if (recorde1 > EEPROM.read(0)) {
           EEPROM.update(0, recorde1);
      }
} 
...
void game_start() {
  ...
  TFT.setCursor( TFTW2 - (12*3.5) - 1, TFTH2 + 34);
...
void game_over() {
  ...
  TFT.setTextSize(2);
  // half width - num char * char width in pixels
  TFT.setCursor( TFTW2 - (9*5), TFTH2 - 4);
  TFT.println("PERDEU!");
  TFT.setTextSize(0);
...
void game_recorde() {
  ...
  TFT.setTextColor(ST7735_WHITE);
  TFT.setTextSize(2);
  TFT.setCursor( TFTW2 - (9*6.5), TFTH2 - 4);
  TFT.println ("Recorde:");
  TFT.setCursor( TFTW2 - (12*2) - 1, TFTH2 + 20);
  TFT.println(EEPROM.read(0));
  TFT.setTextSize(0);
  TFT.setCursor( TFTW2 - (12*4) - 1, TFTH2 + 40);  
